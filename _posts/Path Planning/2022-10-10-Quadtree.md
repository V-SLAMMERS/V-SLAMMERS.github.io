...
## 3. Conclusion
By the way, we can make the 2D grid map much faster and less computation via quadtree. As a result, we can find the shortest path via using A* path finding algorithm and quadtree data structure. Thanks to the [amazing project](github.com/volkerp/quadtree_Astar) by [Volker Poplaswki](github.com/volkerp), we can reduce the time cost for the experiment.
...
Our result is uploaded in the [repository](github.com/V-SLAMMERS/path-planning).